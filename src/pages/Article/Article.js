...
import { getPostById, getUserById } from "../../WebAPI";
...
const Article = () => {
  ...
  const [isLoadingGetPost, setIsLoadingGetPost] = useState(false);
  ...
  useEffect(() => {
    ...
    getPostById(id)
      .then(result => {
        setPost(result)
        getUserById(result.id, result.userId)
          .then(user => {
  ...
  return (
    <ArticleContainer>
      <Wrapper>
        { isLoadingGetPost && <Loading><p>Loading...</p></Loading> }
        ...
        <div>  
          ...
          <ArticleDate>{ !isLoadingGetPost && new Date(post.createdAt).toLocaleString()}</ArticleDate>
        </div>
        <ArticleContent>{post.body}</ArticleContent>
      </Wrapper>
      <ArticleBottom>
        { !isLoadingGetPost && <BackButton onClick={handleBackToLastPage}>返回</BackButton>}
      </ArticleBottom>
    </ArticleContainer>
  )
}
